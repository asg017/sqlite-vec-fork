#!/usr/bin/env python3
"""Full 1M benchmark: baselines (flat, int8, bit) + IVF best configs.

Uses sampled training for IVF (insert 1k, train, insert rest).
"""
import os
import sqlite3
import statistics
...
def run_ivf(name, nlist, nprobe, train_size, out_dir):
    print(f"\n{'='*70}\n  {name}: N={N} nlist={nlist} nprobe={nprobe} train={train_size}\n{'='*70}", flush=True)
    db_path = os.path.join(out_dir, f"{name}.{N}.db")
    if os.path.exists(db_path):
        os.remove(db_path)
    conn = sqlite3.connect(db_path)
    ...
    print(f"    {train_size} vectors in {phase1_time:.1f}s", flush=True)

    # Phase 2: train
    print(f"  Phase 2: Train k-means...", flush=True)
...
def main():
    ...
    # Baselines
    results.append(run_baseline("vec0-flat",
    ...
    results.append(run_baseline("vec0-int8",
        "CREATE VIRTUAL TABLE vec_items USING vec0("
        "  id integer primary key,"
        "  embedding float[768] distance_metric=cosine,"
    ...
    results.append(run_baseline("vec0-bit",
        "CREATE VIRTUAL TABLE vec_items USING vec0("
        "  id integer primary key,"
        "  embedding float[768] distance_metric=cosine,"
    ...
    # IVF configs
    # Training size: max(64*nlist, 10000) — need enough vectors per centroid
    # for meaningful clusters. 64 vectors per centroid is a reasonable minimum.
    import math
    for nlist, nprobe in [(128, 16), (256, 16), (256, 32)]:
        train_size = min(N, max(64 * nlist, 10000))
    ...
    print(f"{'name':>18} {'insert(s)':>10} {'train(s)':>9} {'MB':>6} {'qry(ms)':>8} {'recall':>8}", flush=True)
    print("-" * 65, flush=True)
    for r in results:
        t = f"{r['train_s']:.0f}" if r["train_s"] > 0 else "-"
        print(f"{r['name']:>18} {r['insert_s']:>10.1f} {t:>9} {r['file_mb']:>6.0f} "
