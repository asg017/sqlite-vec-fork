...
def main():
    ...
    for _, row in df_neighbors.iterrows():
        qid = int(row["id"])
        # neighbors_id is a JSON array string
        nids = json.loads(row["neighbors_id"])
        for rank, nid in enumerate(nids):
            rows.append((qid, rank, str(nid)))
    conn.executemany(
    ...
    for start in range(0, total, batch_size):
        ...
        print(f"    {done:>8}/{total}  {elapsed:.0f}s  {rate:.0f} rows/s  eta {eta:.0f}s",
              flush=True)
