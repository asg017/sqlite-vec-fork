#!/usr/bin/env python3
"""Benchmark index configurations: DiskANN vs brute-force baselines.

Compares insert time, DB size, KNN query time, and recall across
DiskANN configs and baseline vec0 tables (float, int8, bit).

Usage:
  # DiskANN vs baselines
  python3 bench_batched_insert.py --subset-size 10000 \
      --configs "diskann-L75:L=75,buffer_threshold=5000" \
                "brute-float:type=float" \
                "brute-int8:type=int8" \
                "brute-bit:type=bit" \
      -o runs/compare

  # DiskANN only, compare L values
  python3 bench_batched_insert.py --subset-size 10000 \
      --configs "L75:L=75" "L128:L=128" \
      -o runs/compare

Config format: name:key=val,key=val
  DiskANN keys: L, R, quantizer (binary|int8), buffer_threshold
  Baseline keys: type (float|int8|bit), oversample (for int8/bit rescore)
  If 'type' is set, it's a baseline. Otherwise it's DiskANN.
"""
import argparse
import os
import sqlite3
...
from bench import load_query_vectors
...
def parse_config(spec):
    ...
    # Determine if baseline or DiskANN
    if "type" in params:
        btype = params["type"]
        ...
        cfg = dict(DISKANN_DEFAULTS)
        for k, v in params.items():
            if k in ("R", "L", "buffer_threshold"):
                cfg[k] = int(v)
            elif k == "quantizer":
                cfg[k] = v
            else:
                raise argparse.ArgumentTypeError(f"Unknown DiskANN key: {k}")
...
def build_diskann(base_db, ext_path, name, params, subset_size, out_dir):
    ...
    return {
        "db_path": db_path,
        "insert_total_s": round(total_time, 3),
...
def build_index(base_db, ext_path, name, params, subset_size, out_dir):
    if params["index_type"] == "diskann":
        return build_diskann(base_db, ext_path, name, params, subset_size, out_dir)
    else:
...
def measure_knn(db_path, ext_path, base_db, params, subset_size, k=10, n=50):
    ...
    for qid, query in query_vectors:
        ...
        if not is_baseline or btype == "float":
            # DiskANN or brute-force float: same MATCH syntax
            ...
            # int8 rescore: coarse search on int8, rescore with float
            results = conn.execute(
                "WITH coarse AS ("
            ...
            # bit rescore: coarse search on bq, rescore with float
            results = conn.execute(
                "WITH coarse AS ("
                "  SELECT id, embedding FROM vec_items"
...
def save_results(out_dir, rows):
    ...
    for r in rows:
        db.execute(
            ...
            "(name, n_vectors, index_type, quantizer, R, L, buffer_threshold, "
            " insert_total_s, insert_per_vec_ms, file_size_mb, "
            " knn_k, knn_n_queries, knn_mean_ms, knn_median_ms, knn_p99_ms, recall) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (r["name"], r["n_vectors"], r["index_type"], r.get("quantizer"),
             r.get("R"), r.get("L"), r.get("buffer_threshold"),
             r["insert_total_s"], r["insert_per_vec_ms"], r["file_size_mb"],
...
def config_description(params):
    """One-line description of a config for the report."""
    if params["index_type"] == "diskann":
        bt = params["buffer_threshold"]
        return (f"diskann  {params['quantizer']:>6}  "
                f"R={params['R']:<3} L={params['L']:<3} "
                f"buf={'%d' % bt if bt > 0 else '-':>5}")
    else:
        btype = params["baseline_type"]
        if btype in ("int8", "bit"):
            return f"brute    {btype:>6}  (rescore oversample={params['oversample']})"
        return f"brute    {btype:>6}"


def print_report(all_results):
    print(f"\n{'name':>16} {'N':>7} {'index config':>38}  "
          f"{'insert(s)':>10} {'ms/vec':>8} {'MB':>8} "
          f"{'qry(ms)':>8} {'recall':>8}")
    print("-" * 120)
    for r in all_results:
        print(f"{r['name']:>16} {r['n_vectors']:>7} {r['config_desc']:>38}  "
              f"{r['insert_total_s']:>10.1f} {r['insert_per_vec_ms']:>8.2f} "
              f"{r['file_size_mb']:>8.1f} "
              f"{r['knn_mean_ms']:>8.2f} {r['recall']:>8.4f}")


def main():
    parser = argparse.ArgumentParser(
        description="Benchmark DiskANN vs baseline index configurations",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Config format: name:key=val,key=val\n\n"
            "DiskANN keys: L, R, quantizer (binary|int8), buffer_threshold\n"
            "  Example: fast:L=75,buffer_threshold=5000\n\n"
            "Baseline keys: type (float|int8|bit), oversample (default 8)\n"
            "  Example: brute-float:type=float\n"
            "  Example: brute-int8:type=int8,oversample=10"
        ),
    )
    ...
    for i, (name, params) in enumerate(configs, 1):
        ...
        print(f"  Build: {build['insert_total_s']}s  "
              f"{build['insert_per_vec_ms']}ms/vec  "
              f"{build['file_size_mb']} MB")

        knn = {"knn_mean_ms": 0, "knn_median_ms": 0, "knn_p99_ms": 0,
               "recall_at_k": 0, "k": args.k, "n_queries": args.n}
        if not args.skip_recall:
            ...
            print(f"  KNN: mean={knn['knn_mean_ms']}ms  "
                  f"recall@{args.k}={knn['recall_at_k']}")
        ...
        row = {
            ...
            "config_desc": desc,
            "insert_total_s": build["insert_total_s"],
        ...
        # Add DiskANN-specific fields
        if params["index_type"] == "diskann":
            row["quantizer"] = params["quantizer"]
            row["R"] = params["R"]
            row["L"] = params["L"]
            row["buffer_threshold"] = params["buffer_threshold"]
